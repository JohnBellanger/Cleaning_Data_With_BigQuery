...
CREATE TABLE `marketing-464513.Finance.Sales_Backup_2` AS
SELECT*
FROM `marketing-464513.Finance.Sales`
;
...
CREATE OR REPLACE TABLE `marketing-464513.Finance.Sales_Backup_2` AS
...
UPDATE `marketing-464513.Finance.Sales_Backup_2` # ok
...
CREATE OR REPLACE TABLE `marketing-464513.Finance.Marketing_Monthly` AS

SELECT  FORMAT_DATE('%Y-%m', Date) AS Date,
        Marketing_Source,  
        Campaign_Name,   
...
GROUP BY Date, Marketing_Source, Campaign_Name
;

# Step 2: Aggregation for Sales

CREATE OR REPLACE TABLE `marketing-464513.Finance.Sales_Monthly` AS

SELECT  count(DISTINCT(Number_Order)) AS Number_Order,
...
FROM `marketing-464513.Finance.Sales_Backup_2`
GROUP BY Date, Traffic_Source, Product_Category
;

SELECT*
FROM `marketing-464513.Finance.Sales_Monthly`

# Phase 8: Creation of flat table

CREATE OR REPLACE TABLE `marketing-464513.Finance.Sales_Flat_table_Monthly` AS

SELECT  Coalesce(m.Date,s.Date) AS Date, 
        Coalesce(m.Marketing_Source, s.Traffic_Source) AS Traffic_Source,
        IFNULL(m.Campaign_Name,"Organic / Unknown") AS Campaign_Name,
        IFNULL(s.Product_Category, "No Sales") AS Product_Category,
...
FROM  `marketing-464513.Finance.Marketing_Monthly` m
FULL OUTER JOIN `marketing-464513.Finance.Sales_Monthly` s
ON m.Date = s.Date
...
SELECT *
FROM `marketing-464513.Finance.Sales_Flat_table_Monthly`
;


# Phase 8: DEBUG

# Step 1: I count the row - 175 rows ok
